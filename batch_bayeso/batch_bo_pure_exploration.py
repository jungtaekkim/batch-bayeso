...
class BBOPureExploration(base_batch_bo.BaseBBO):
    def __init__(self, range_X: np.ndarray,
        ...
        self.dummy_value = 1.0 # It does not matter to pure exploration.
    ...
    def compute_acquisitions(self, X: np.ndarray,
        ...
        fun_acquisition = utils_bo.choose_fun_acquisition(str_acq, hyps.get('noise', None))
    ...
    def optimize(self, X_train: np.ndarray, Y_train: np.ndarray,
        ...
        next_point, next_points = self._optimize(fun_negative_acquisition,
            str_sampling_method=str_sampling_method, num_samples=num_samples, seed=seed)
        ...
        X_train = np.concatenate([X_train, [next_point]], axis=0)
        Y_train = np.concatenate([Y_train, [[self.dummy_value]]], axis=0)
        next_points_all = next_points
        acquisitions_all = acquisitions

        for ind in range(1, self.size_batch):
            time_start_surrogate = time.time()
