...
class BBOPrediction(base_batch_bo.BaseBBO):
    ...
    def optimize(self, X_train: np.ndarray, Y_train: np.ndarray,
        ...
        for ind in range(0, self.size_batch):
            ...
            prediction_mean, _ = self.compute_posteriors(X_train, Y_train,
                next_point[np.newaxis, ...],
                cov_X_X, inv_cov_X_X, hyps)

            X_train = np.concatenate([X_train, [next_point]], axis=0)
            Y_train = np.concatenate([Y_train, [prediction_mean]], axis=0)
